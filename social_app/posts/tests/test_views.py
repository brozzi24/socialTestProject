...
class TestViews(TestCase):
    def setUp(self):
        ...
        self.post1 = Post.objects.create(
            author=self.user,
            text="Do you know what they call a quarter pounder in europe?",
        )
    ...
    def test_createPost_valid(self):
        ...
        response = self.client.post(
            self.createPost, data={"text": "be alot cooler if you did",}
        )
    ...
    def test_deletePost(self):
        ...
        response = self.client.post(self.deletePost, data={"post_id": post2.id,})
    ...
    def test_createComment_valid(self):
        ...
        response = self.client.post(
            self.createComment,
            data={"text": "be alot cooler if you did", "post_id": self.post1.id,},
        )
        self.assertRedirects(response, "/", status_code=302)
    ...
    def test_createComment_user_logged_out(self):
        comment_count = Comment.objects.count()
        response = self.client.post(
            self.createComment, data={"text": "be alot cooler if you did",}
        )
    ...
    def test_deleteComment(self):
        comment = Comment.objects.create(
            author=self.user, text="hello", post=self.post1
        )
        comment_count = Comment.objects.count()
        login = self.client.login(username="testuser", password="brandon24")

        response = self.client.post(
            self.deleteComment, data={"comment_id": comment.id,}
        )
